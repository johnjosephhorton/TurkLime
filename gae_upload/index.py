...
class MainHandler(webapp.RequestHandler):
    def get(self):
        ...
        temp = os.path.join(
            ...
            'templates/create_experiment.htm')        
...
# Experimenter see this screen before the actual program is launched on 
# MTurk. 
    ...
    def get(self, raw_resource): 
        ...
        if create_hit_rs[0].IsValid == 'True': 
            outstr = template.render(temp, 
                                 {'message': """Your HIT was created. </br> 
                                                The HITId is %s </br>  
                                             <a href="/">Input another YAML</a>"""%create_hit_rs[0].HITId})
        else: 
            outstr = template.render(temp, 
                                 {'message': "Your HIT was not created"})
        self.response.out.write(outstr)
...
class BackToTurk(webapp.RequestHandler): 
    """This is called when the user clicks on the submit link on limesurvey"""
    def get(self): 
